<template>
  <div v-if="!removed" :style="{ 'height': height }" class="element row" :class="{
    ...
    <div v-if="previewing" class="ml-5 p-5">
      {{ fill }}
    </div>
    <textarea v-else-if="!reviewing && !filling" type="text" class="form-control ml-5 p-5" readonly></textarea>
    <textarea v-else type="text" class="form-control ml-5 p-5" v-model="fill" :required="required"
      :class="{ 'border-danger': showRequiredNotify && required && !fill }" :disabled="reviewing || !enabled"></textarea>
