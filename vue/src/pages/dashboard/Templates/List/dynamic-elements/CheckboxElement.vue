<template>
  <div v-if="!removed" class="element row" :class="{
    ...
    <div v-else>
      <div v-for="child in childList" class="col-12 row row mt-1 mb-1">
        <label :for="child.text" class="col">
          <input type="checkbox" :id="child.text" :required="required" :checked="fill !== null && fill.match(child.text)"
            :value="child.text" @change="toggleChoice($event, child.text)" :disabled="reviewing || !enabled">
