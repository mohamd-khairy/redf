<template>
  ...
  >
    ...
    <div
      class="col-1 cursor-pointer pt-0"
    ...
    <div v-if="previewing">
      <div v-for="child in childList" class="col-12 row row mt-1 mb-1">
        <label :for="child.text" class="col">
          <span
            v-if="fill !== null && fill.match(child.text)"
    ...
    <div v-else-if="!reviewing && !filling">
      <div v-for="(child, i) in childList" class="col-12 row mt-1 mb-1 p-2">
        ...
        <textarea
          type="text"
          v-model="child.text"
          class="border-0 col ml-5 p-0 background-0"
        ></textarea>
        <span
          v-if="i"
          class="material-icons text-primary col-1 cursor-pointer"
          @click="childList.splice(childList.indexOf(child), 1)"
        >
          <i class="v-icon notranslate mdi mdi-minus"></i>
        </span>
        <span
          v-else
          class="material-icons text-success col-1 cursor-pointer"
          @click="addChoice"
        >
      ...
      <div v-for="child in childList" class="col-12 row row mt-1 mb-1">
        <label :for="child.text" class="col">
          <input
            ...
            :id="child.text"
            :required="required"
            :checked="fill !== null && fill.match(child.text)"
            :value="child.text"
    ...
    <ul
      class="context-menu-list context-menu-root"
      v-if="menuOpen"
      v-on:clickout="menuOpen = false"
    >
      <li
        class="context-menu-item cursor-pointer text-danger"
        @click="confirmRemove"
      >
        <i class="v-icon notranslate mdi mdi-delete-forever"> </i>
        <span class="mx-2">Remove</span>
      </li>
      <li class="context-menu-item">
        <i class="v-icon notranslate mdi mdi-wrench-outline"> </i>
      ...
      <li class="context-menu-item">
        ...
        <i
          class="cursor-pointer v-icon notranslate mdi mdi-format-align-left"
          @click="notes = 'left'"
        >
        </i>
        <i
          class="cursor-pointer v-icon notranslate mdi mdi-format-align-center"
          @click="notes = 'center'"
        >
        </i>
        <i
          class="cursor-pointer v-icon notranslate mdi mdi-format-align-right"
          @click="notes = 'right'"
        >
        </i>
      </li>
      <li class="context-menu-item">
        <i class="v-icon notranslate mdi mdi-earth"> </i>
...
export default {
  ...
  methods: {
    ...
    toggleChoice(e, text) {
      if (e.target.checked)
        if (this.fill == null) this.fill = text + ",";
        else this.fill += text + ",";
      else this.fill = this.fill.replace(text + ",", "");
    },
    confirmRemove() {
