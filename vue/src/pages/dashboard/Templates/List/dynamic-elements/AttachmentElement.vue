<template>
  <div v-if="!removed" class="element row" :class="{
    ...
    <div v-if="!reviewing && !filling">
      <div v-for="(attachment, i) in childList" class="col-12 row mt-1 mb-1 p-2">
        <select v-model="attachment.type" class="form-control col">
          <option value="image">Image</option>
          <option value="pdf">PDF</option>
          <option value="word">Word</option>
        </select>
        <!--        <span v-if="i" class="material-icons text-primary col-1 cursor-pointer" @click="childList.splice(childList.indexOf(attachment), 1)">-->
        <!--          remove circle-->
        <!--        </span>-->
        <!--        <span v-else class="material-icons text-success col-1 cursor-pointer" @click="addAttachment">-->
        <!--          add circle-->
        <!--        </span>-->
      </div>
    </div>
    <div v-else>
      <div v-for="(attachment, index) in childList" class="col-12 row mt-1 mb-1 p-2">
        <div>
          Attachment ({{ attachment.type }}) <input type="file" :id="'files-' + id + '-' + index">
          <span class="text-danger" v-if="attachment.error">{{ attachment.error }}</span>
        </div>
      </div>
